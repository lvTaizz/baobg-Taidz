...
local block = Instance.new("Part", workspace) -- Skid redz nhung ma ngu vai lon
block.Size = Vector3.new(1, 1, 1)
block.Name = "Thang lon cat hup"
block.Anchored = true
...
task.spawn(function()
    ...
    Toggle = Tabs.Settings:AddToggle("MyToggle", {Title = "Bypass Tp", Default = false })
    Toggle:OnChanged(function(Value)
        BypassTP = Value		
        saveSettings() 
    end)
...
spawn(function()
    while wait() do 
        if BoneFMode == "No Quest" and _G.Auto_Bone and Sea3 then
            pcall(function()
...
spawn(function()
    while wait() do
        if BoneFMode == "Quest" and _G.Auto_Bone and Sea3 then
            pcall(function()
                local QuestTitle = game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text
...
spawn(function()
    while wait() do
        if CakeFMode == "No Quest" and _G.AutoDoughtBoss and Sea3 then
...
task.spawn(function()
    while wait() do
        if CakeFMode == "Quest" and _G.AutoDoughtBoss and Sea3 then
            pcall(function()
...
local Elite_Hunter_Status = Tabs.stack:AddParagraph({
 ...
 Toggle = Tabs.stack:AddToggle("MyToggle", {Title = "Auto Elite Hunter", Default = _G.AutoElitehunter })
    ...
    spawn(function()
        while wait() do
            if _G.AutoElitehunter and Sea3 then
                pcall(function()
...
local piranhatoggle = Tabs.Se:AddToggle("piranhatoggle", {Title = "Auto Kill Piranha",Description = "Enable for kill Piranha if have", Default = _G.AutoKillPiranha })
    ...
    --     while wait() do
    --         pcall(function()
    --             if _G.AutoKillShark and  Sea3 then
    --                 for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
    --                     if v.Name == "Shark" then
    ...
    --     while wait() do
    --         pcall(function()
    --             if _G.AutoKillPiranha and Sea3 then
    --                 for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
    --                     if v.Name == "Piranha" then
    --                         if (v.HumanoidRootPart.CFrame.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame.Position).Magnitude <= 500 then
    --                         if game:GetService("Workspace").Enemies:FindFirstChild(v.Name) then
    ...
    --     while wait() do
    --         pcall(function()
    --             if _G.AutoKillFishCrew and  Sea3 then
    --                 for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
    --                     if v.Name == "Fish Crew Member" then
    --                         if (v.HumanoidRootPart.CFrame.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame.Position).Magnitude <= 500 then
    --                         if game:GetService("Workspace").Enemies:FindFirstChild(v.Name) then
    ...
    -- spawn(function()
    --     while wait() do
    --         if  _G.AutoTerrorshark and Sea3 then
    --             pcall(function()
    --                 if game:GetService("Workspace").Enemies:FindFirstChild("Terrorshark") then
    --                     for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
    --                         if v.Name == "Terrorshark" then
    ...
    while wait() do
        if _G.AutoBuyLegendarySword then
            pcall(function()
                ...
                if _G.AutoBuyLegendarySword_Hop and _G.AutoBuyLegendarySword and Sea2 then
...
spawn(function()
        while wait() do
            if  _G.Autotushita and Sea3 then
                pcall(function()
...
local cooldowntickFire = 0
local MaxFire = 1000
local FireCooldown = 0.6 -- Giảm thời gian delay để tấn công nhanh hơn
local FireL = 0
local bladehit = {}
...
CancelCoolDown = LPH_JIT_MAX(function()
	local ac = CombatFrameworkR.activeController
	if ac and ac.equipped then
		AttackCoolDown = tick() + (FireCooldown or 0.01) -- Điều chỉnh thời gian chờ giữa các đợt tấn công
		RigEven.FireServer(RigEven,"weaponChange",ac.currentWeaponModel.Name)
		FireL = FireL + 1
		task.delay((FireCooldown or 0.01),function()
			FireL = FireL - 1
		end)
	end
end)
...
AttackFunction = LPH_JIT_MAX(function(typef)
	...
	if ac and ac.equipped then
		...
		if #bladehit > 0 then
			...
			if tick() - cooldowntickFire > 0.01 then -- Tăng tần suất đòn đánh
				ac.timeToNextAttack = 0 -- Tấn công liên tục không delay
				ac.hitboxMagnitude = 60 -- Kích thước hitbox lớn hơn
				pcall(task.spawn,ac.attack,ac)
				cooldowntickFire = tick()
			end
			...
			AttackAnim.AnimationId = REALID
			local StartP = ac.humanoid:LoadAnimation(AttackAnim)
			StartP:Play(0.01,0.01,0.01) -- Animation diễn ra rất nhanh
			RigEven.FireServer(RigEven,"hit",bladehit,AMI3 and 4 or 5,"")
			task.delay(0.01,function() -- Giảm thời gian giữa các lần đánh
				StartP:Stop()
			end)
		end
	end
...
local LastAttackTime = 0 -- Biến lưu thời gian tấn công cuối cùng
Fast_Attack = true
NeedAttacking = true
...
spawn(function()
    while game:GetService("RunService").Stepped:Wait() do
        ...
        if ac and ac.equipped and not CheckStun() then
            if NeedAttacking and Fast_Attack then
                -- Kiểm tra nếu đã tấn công liên tục trong 5 giây thì nghỉ 1 giây
                if tick() - LastAttackTime >= 5 then
                    task.wait(1) -- Nghỉ 1 giây
                    LastAttackTime = tick() -- Cập nhật lại thời gian tấn công cuối cùng
                end

...
repeat task.wait() until game.Players -- Wait until Players is available
repeat task.wait() until game.Players.LocalPlayer -- Wait for LocalPlayer

    repeat task.wait() -- Loop until team is set
    pcall(function()
...
until tostring(game.Players.LocalPlayer.Team) == "Pirates" -- Ensure the team is Pirates

repeat task.wait() until game.Players.LocalPlayer.Character -- Wait for the character to load
...
local function AttackFunction(typef)
    ...
    if ac and ac.equipped then
        ...
        if #bladehit > 0 then
            ...
            local REALID = ac.anims.basic[3] or ac.anims.basic[2] or "" -- Đảm bảo có giá trị hợp lệ
